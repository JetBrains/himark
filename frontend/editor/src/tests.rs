    ...
    for source in ["hello\nworld\n", "line one\nline two", "\n\n", "abc"] {
        ...
        while !text.is_empty() {
            ...
            while let Some(pending) = layout.repair_pending() {
                rounds += 1;
                assert!(rounds < 64, "src={source:?} at {at}: chain did not terminate");
                layout.repair_layout_bounded(
                    &value,
                    markup,
                    600.0,
        ...
        while let Some(pending) = layout.repair_pending() {
            rounds += 1;
            assert!(rounds < 64, "src={source:?}: resize chain did not terminate");
            let _ = anchor;
            layout.repair_layout_bounded(
                &empty,
                markup,
...
fn tight_background_markers_hug_their_glyphs() {
    ...
    let banded_rows = |bytes: &[u8]| -> Vec<usize> {
        (0..80)
            .filter(|row| bytes[row * row_bytes..(row + 1) * row_bytes] != plain[row * row_bytes..(row + 1) * row_bytes])
            .collect()
    };
...
fn inline_marker_styles_are_tight_and_line_washes_are_not() {
    ...
    for theme in [crate::theme::Theme::embedded(), crate::theme::Theme::light()] {
        for id in [
            StyleId::BraceMatch,
            StyleId::Occurrence,
            StyleId::InlineCode,
...
fn lazy_languages_load_on_first_parse_only() {
    ...
    languages.register_lazy(
        ...
        None,
        Arc::new(move || {
            match failing.fetch_add(1, Ordering::SeqCst) {
                0 => None,
...
fn deleting_the_text_drops_its_markup_intervals() {
    ...
    assert_eq!(markup.interval_count(), 0, "and none is left in the key map");
}
