...
impl imba::View for CommitTip {
    ...
    fn layout<'a>(
        ...
        imba::leaf::leaf::<Self::Command>(width.min(560.0), height)
            .paint_instead(move |_arena, canvas, rect| {
                ...
                for (line, dim) in &lines {
                    paint.set_color(if *dim { colors.dim_text.0 } else { colors.text.0 });
...
impl HistoryView {
    ...
    pub(crate) fn searching(&self) -> bool {
        self.list.view().searching()
    }

    pub(crate) fn clear_search(&mut self, fx: &mut Effects<'_, HistoryCommand>) {
        fx.scope(
            |command| HistoryCommand::Rows(TooltipCommand::Host(command)),
            |fx| self.list.view_mut().clear(fx),
        );
    }
    ...
    pub(crate) fn refresh(&mut self, store: &Store) {
        ...
        let accent = ui_theme.peeker.accent.0;
        let dim = ui_theme.peeker.dim_text.0;
        ...
        for (key, item) in items.iter() {
            if matches!(item, RowItem::Commit { .. }) && !self.items.contains_key(key) {
                self.list.view_mut().inner_mut().forest.preset_collapsed(key);
            }
        }
        self.items = items;
    ...
    fn activate_key(&mut self, key: &ResourceLocation) {
        if !matches!(self.items.get(key), Some(RowItem::Note) | None) {
            self.list.view_mut().inner_mut().list_mut().select_only(key.clone());
        }
...
impl View for HistoryView {
    ...
    fn perform(
        ...
        match command {
            ...
            HistoryCommand::Select(delta) => self.list.view_mut().inner_mut().list_mut().cursor_step(delta),
