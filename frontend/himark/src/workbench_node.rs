    ...
    fn perform(
        ...
        match (self, command) {
            ...
            (Self::Leaf(slot), NodeCommand::Leaf(PanelCommand::Hover(found))) => fx
                .scope(NodeCommand::Leaf, |fx| slot.land_hover(store, ui, found, fx)),
            ...
            (Self::Leaf(slot), NodeCommand::Leaf(command)) => {
                ...
                )) = &command
                {
                    let point = *point;
                    return fx
                        .scope(NodeCommand::Leaf, |fx| slot.sync_hover(store, ui, point, fx));
                }
        ...
        if matches!(self, Self::Leaf(slot) if slot.hover.armed()) {
            return imba::ThunkBox::new(
                ...
                widget.event(|_arena, event, _size| match event {
                    imba::event::Event::AnimationClock { now } => imba::event::EventResult::Command(
                        NodeCommand::Leaf(PanelCommand::HoverTick(*now)),
                    ),
