...
fn a_failed_run_commits_nothing() {
    ...
    let landing = ran(launched(&mut store, script));
    landed(&mut store, script, landing);
    assert_eq!(text_of(&store, plan), "alpha", "the half-run committed nothing");
    let runs = ScriptRuns::of(&store);
    assert!(runs[0]
        .error
        .as_deref()
...
fn a_failed_turn_fails_the_run() {
    ...
    landed(&mut store, script, landing);
    assert_eq!(text_of(&store, plan), "old", "the half-run committed nothing");
}
...
fn shows_file_now_or_ride_their_store() {
    ...
    assert!(queued(&store), "the deferred show queued on the store's landing");
    store.put(himark::AppRequests::default());
    stored_landing(
        &mut store,
        ScriptStored {
