...
#[derive(Clone, Default)]
pub struct ScriptLanes(
    pub std::collections::HashMap<String, imba::effect::CancellationToken>,
);
    ...
    async fn handle(&self, effect: RunScriptEffect) -> ScriptLanding {
        ...
        for write in outcome.writes {
            let Some(location) = resolve(&capture.base, &write.target) else {
                log.push(format!("write dropped: unresolvable path {:?}", write.target));
                continue;
            };
            match snapshots
                .iter()
...
impl himark::DynamicEditorCommand for RunScript {
    ...
    fn perform(
        ...
        let agent = himark::Windows::list(store).into_iter().next().and_then(|window| {
