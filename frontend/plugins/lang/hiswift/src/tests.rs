...
fn highlights() {
    let document = parsed();
    assert_eq!(style_at(&document, "func"), Some(StyleId::Keyword), "func");
...
fn outline() {
    ...
    assert_eq!(
        titles,
        vec!["func greet", "class Point", "func norm", "protocol Shape"]
    );
}
...
fn folds() {
    ...
    assert!(texts[1].contains("func norm"), "fold 1: {:?}", texts[1]);
    assert!(texts[2].contains("return 0"), "fold 2: {:?}", texts[2]);
    assert!(texts[3].contains("area"), "fold 3: {:?}", texts[3]);
}
