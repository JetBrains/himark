...
fn outline() {
    ...
    assert_eq!(
        titles,
        vec!["class Point", "def norm", "object Main", "def main"]
    );
}
...
fn folds() {
    ...
    let texts: Vec<&str> = folds
        ...
        .collect();
    assert_eq!(texts.len(), 4, "fold interiors: {texts:?}");
    assert!(texts[0].contains("def norm"), "fold 0: {:?}", texts[0]);
    assert!(texts[1].contains("val c"), "fold 1: {:?}", texts[1]);
    assert!(texts[2].contains("def main"), "fold 2: {:?}", texts[2]);
    assert!(texts[3].contains("println"), "fold 3: {:?}", texts[3]);
}
