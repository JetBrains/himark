...
fn highlights() {
    let document = parsed();
    assert_eq!(style_at(&document, "fun"), Some(StyleId::Keyword), "fun");
...
fn outline() {
    ...
    let titles: Vec<&str> = items
        ...
        .collect();
    assert_eq!(titles, vec!["fun greet", "class Point", "fun norm"]);
}
...
fn folds() {
    ...
    assert!(texts[1].contains("fun norm"), "fold 1: {:?}", texts[1]);
