...
fn outline() {
    ...
    assert_eq!(
        titles,
        vec!["namespace Geo", "class Point", "int Norm", "string Label"]
    );
}
...
fn folds() {
    ...
    assert!(texts[0].contains("class Point"), "fold 0: {:?}", texts[0]);
    assert!(texts[1].contains("Norm"), "fold 1: {:?}", texts[1]);
