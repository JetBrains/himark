...
contract Point {
    ...
    function norm() public pure returns (uint) {
        return 0;
    }
}
"#;
...
fn outline() {
    ...
    let titles: Vec<&str> = items
        ...
        .collect();
    assert_eq!(titles, vec!["contract Point", "function norm"]);
}
...
fn folds() {
    ...
    assert!(texts[0].contains("norm"), "fold 0: {:?}", texts[0]);
    assert!(texts[1].contains("return 0"), "fold 1: {:?}", texts[1]);
}
