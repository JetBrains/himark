...
pub(crate) fn with_abi_location<R>(
    location: &ResourceLocation,
    with: impl FnOnce(*const HimarkLocation) -> R,
) -> R {
    let segments: Vec<HimarkStr> = location
        .path()
        .iter()
        .map(|segment| HimarkStr::of(segment))
        .collect();
    let abi = HimarkLocation {
        kind: HimarkStr::of(location.kind().as_str()),
        authority: HimarkStr::of(location.authority().as_str()),
