...
fn document_location(path: &[&str]) -> himark::ResourceLocation {
...
fn a_caret_move_lights_the_bracket_pair_in_an_opened_rust_file() {
    ...
    settle_until(&mut engine, "the bracket pair lit", |engine| {
        ...
        himark::OpenDocuments::list(store).into_iter().any(|entity| {
            let document = entity.1.document();
...
fs_probe!(
    probe_list,
    himark::ListDirectoryEffect,
    Option<Vec<himark::ResourceLocation>>,
    |location: &himark::ResourceLocation| himark::ListDirectoryEffect {
...
fn probe_store(
    ...
    let slot: std::sync::Arc<std::sync::Mutex<Option<bool>>> =
        std::sync::Arc::new(std::sync::Mutex::new(None));
    let filled = std::sync::Arc::clone(&slot);
    ...
    engine.app.perform_batch(vec![himark::AppCommand::Dynamic(
        ...
        std::sync::Arc::new(LaunchProbe {
            launch: Box::new(move |window, fx| {
                let filled = std::sync::Arc::clone(&filled);
                let _ = fx.push(
                    imba::effect::AnyEffect::new(himark::StoreDocumentEffect {
                        location: location.clone(),
                        text: text.clone(),
                    })
                    .map(move |stored| {
                        himark::AppCommand::Dynamic(
                            ...
                            std::sync::Arc::new(FillProbe {
                                slot: std::sync::Arc::clone(&filled),
                                value: std::sync::Mutex::new(Some(stored)),
                            }),
                        )
                    }),
                );
...
fn the_wire_serves_a_session_filesystem() {
    ...
    let wire: std::sync::Arc<dyn himark::higent::AhpServer> = std::sync::Arc::new(
        crate::hiahp::wire::WireHost::new(crate::hiahp::wire::test_runtime(), crate::test_connector()),
    );
...
fn the_seat_speaks_ahp_over_the_http_face() {
    ...
    let seat = crate::hiahp::wire::WireHost::at(crate::hiahp::wire::test_runtime(), crate::test_connector(), ws);
...
fn the_floating_chat_rides_the_bottom_sheet() {
    ...
    let band = himark::Windows::window_ref(engine.app.store(), engine.app.sole_window())
        .and_then(|entity| {
            entity.bottom_rect(engine.app.store(), skia_safe::Size::new(1100.0, 800.0))
        })
        .expect("the sheet's rectangle");
    let _ = himark::test_driver::click(
        &mut engine.app,
        (band.left + band.right) * 0.5,
        band.top + 20.0,
        1100.0,
        800.0,
    );
    ...
    assert_eq!(
        ...
        "the band click moved the keys to the sheet"
    );
    ...
    let _ = himark::test_driver::click(&mut engine.app, 550.0, 700.0, 1100.0, 800.0);
    pump(&mut engine, &mut surface);
    assert_eq!(
        floating(&engine).map(|chat| chat.blurred()),
        Some(false),
        "a click into the band grows the prompt back"
    );
...
fn the_chat_runs_through_the_himark_host() {
    ...
    let plus = plus.expect("the himark host row connected");
    pick_drawer_row(&mut engine, plus);
    let chat_ready =
        |engine: &HimarkEngine| -> bool { shown_chat(engine).is_some_and(|chat| chat.ready()) };
    let mut waited = 0;
    ...
    assert!(chat_ready(&engine), "the chat over our host subscribed");
    ...
    assert!(
        replied(&engine),
        "the reply streamed home: {:?}; host log:\n{}",
    ...
    let mut waited = 0;
    while !ask_up(&engine) && waited < 100 {
        pump(&mut engine, &mut surface);
        waited += 1;
    }
    assert!(ask_up(&engine), "the ask card raised");
    ...
    let mut waited = 0;
    while !shows(&engine, "attached review comment") && waited < 100 {
        pump(&mut engine, &mut surface);
        waited += 1;
    }
    assert!(
        shows(&engine, "attached review comment"),
    ...
    let mut waited = 0;
    while !shows(&engine, "sent while hidden") && waited < 100 {
        pump(&mut engine, &mut surface);
        waited += 1;
    }
    assert!(
        shows(&engine, "sent while hidden"),
...
fn the_session_workspace_lists_and_opens_files_through_the_himark_host() {
    ...
    settle_until(&mut engine, "the served open seated its sync loop", |engine| {
        hiahp::docsync::SyncSeats::count(engine.app.store()) == 1
    });
    assert!(engine.perform_command(window, "workbench.close"));
    pump(&mut engine, &mut surface);
    assert_eq!(
...
fn a_drain_hands_the_ui_thread_back_mid_burst() {
    ...
    assert!(engine.drain());
    assert_eq!(engine.queued_landings(), 0, "the burst finished in one pass");
}
...
fn hover_rest_mounts_a_markdown_popup_over_the_word() {
    let (_host, mut engine, window, fs) = hosted_engine();
    let mut surface = skia_safe::surfaces::raster_n32_premul((900, 700)).expect("surface");
    open_picked(&mut engine, window, &fs, &["hover.rs"], "let value = other;\n");
    let _ = engine.draw(window, surface.canvas(), 900.0, 700.0, 1.0);
    ...
    assert!(engine.mouse_move(window, x + w * 0.5, y + h * 0.5));
    rest(&mut engine);
    settle_until(&mut engine, "the card stood before the cover", popup_standing);
    assert!(engine.perform_command(window, "peeker.toggle"));
    settle(&mut engine);
