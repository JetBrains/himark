...
static bool parse_pipe_table(Scanner *s, TSLexer *lexer,
    ...
    for (;;) {
        ...
        if (lexer->lookahead == '|') {
            // PATCHED (himark): an empty delimiter cell disqualifies the
            // row — GFM requires at least one dash per cell. Upstream
            // counted it as valid, which let a body row of empty cells
            // (`| 1 | 2 |` then `|   |   |`) register as a NEW table's
            // header + delimiter and derail the parse of everything
        ...
        if (lexer->lookahead == '|') {
            // PATCHED (himark): see above — a dash-less delimiter cell is
