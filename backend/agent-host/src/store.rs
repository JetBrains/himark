...
pub struct Manifest {
    ...
    /// Where the current title came from, so a model-generated title
    /// upgrades a prompt-derived one exactly once and never the other
    /// way around.
    #[serde(default, skip_serializing_if = "TitleSource::is_prompt")]
...
pub enum TitleSource {
    /// Truncated first user prompt — the placeholder until a model title lands.
    #[default]
    Prompt,
    /// Generated by a one-shot model call after the first turn completed.
    Model,
}

impl TitleSource {
    fn is_prompt(&self) -> bool {
        matches!(self, TitleSource::Prompt)
    }
}
