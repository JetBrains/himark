...
together as read-only specimens: Air typography roles, surface variants,
primary/secondary/ghost buttons in default, hovered, pressed, focused and
disabled states, unchecked/checked/indeterminate checkboxes, list-row styles
and states, and expanded/collapsed/leaf tree items. Interactive buttons retain
...
using the Air kit's embedded Inter and JetBrains Mono variable fonts.
Linux is the reference platform for pixel comparisons; those four tests are
...
measurements from Chromium rendering the kit's original SCSS, fonts and
component DOM. Width tolerance is 1/32px (browser layout rounding); heights
must match exactly. The browser/native PNGs also support visual review;
font antialiasing differs between Chromium and Skia, so these comparisons
are not a claim of byte-identical rendering.

To reproduce the reference outside the private frontend monorepo's build:

```sh
npm install --prefix /tmp/air-reference-tools sass@1.97.3 playwright@1.58.2
node /tmp/air-reference-tools/node_modules/playwright/cli.js install chromium
node scripts/air-ui-reference.cjs /path/to/jcp-frontend /tmp/air-reference-tools /tmp/air-reference
HIMARK_SHOT=/tmp/air-reference cargo test -p higallery --lib air_reference_specimens
```

`AIR_CHROMIUM_PATH` may select an existing Chromium executable. The script
writes web screenshots and measurements; the Rust test exports the matching
native specimens. Browser measurements in `tests/air-ui` are source fixtures,
not regenerated by `HIMARK_UPDATE_GALLERY`.
